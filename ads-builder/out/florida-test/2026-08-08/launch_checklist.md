# Launch checklist — Sunshine State Movers
...
- [x] GTM container installed on https://sunshinestatemovers.example — `GTM-XXXX123`
- [x] GA4 property created and linked to Google Ads — `G-ABC123XYZ`
- [x] Call tracking live, with dynamic number insertion on the site
- [ ] Tracking number is in **Sunshine State Movers's** name, not the agency's
...
- [ ] Add the call asset using the **tracking** number, not 305-555-0199
...
- [ ] Deploy `site/` to **go.https://sunshinestatemovers.example**
- [ ] Ask the client to add one DNS record: `go.https://sunshinestatemovers.example` CNAME → your Pages project
...
Generated for Sunshine State Movers. 5 campaigns, 398.75 USD/day. Campaigns paused, ad groups enabled — so going live is one switch per campaign.
