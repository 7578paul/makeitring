...
- [ ] GTM container installed on https://www.movingpapa.com — **container ID still needed**
...
- [ ] Tracking number is in **Moving Papa's** name, not the agency's
...
- [ ] Ad URLs currently point at the client's own site. If you deploy `site/` to **go.movingpapa.com**, update the campaigns' Final URLs to match — one destination, everywhere
- [ ] Deploy `site/` to **go.movingpapa.com**
- [ ] Ask the client to add one DNS record: `go.movingpapa.com` CNAME → your Pages project
...
Generated for Moving Papa. 19 campaigns, 2,302.63 CAD/day. Campaigns paused, ad groups enabled — so going live is one switch per campaign.
