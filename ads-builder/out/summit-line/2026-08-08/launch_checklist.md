# Launch checklist — Summit Line Movers
...
- [ ] GTM container installed on https://summitlinemovers.com — **container ID still needed**
...
- [ ] Tracking number is in **Summit Line Movers's** name, not the agency's
...
- [ ] Deploy `stack/` once: `wrangler d1 create makeitring-leads`, run `schema.sql`, set `CLIENT_KEYS`, `wrangler deploy`
- [ ] Point this client's form at the worker: `/lead?client=summit-line`
- [ ] Point the CallRail post-call webhook at `/webhook/callrail?client=...` — caller, duration and recording then appear on the dashboard
- [ ] If `gtm-container.json` is in this package: GTM → Admin → **Import Container** → merge. Two clicks instead of script installs
- [ ] Weekly: download `/export/conversions.csv` from the dashboard and upload in Google Ads → Conversions → Upload — booked jobs with real values are what teach bidding to chase revenue
- [ ] Weekly: download `/export/ip-exclusions.csv` and paste into Campaign Settings → IP exclusions — the self-hosted click-fraud layer
...
- [ ] Optional: the negative wall is already in the import, applied per campaign. `shared_negative_list.csv` is the same wall as one file if you later prefer to manage it as a shared library list
- [ ] Add the call asset once the **tracking** number exists — the import ships without one on purpose, so the public number never ends up in ads unattributable
- [ ] Performance Max: add marketing images (landscape, square, portrait) and a logo to each asset group — images cannot ship in a CSV, and the asset group will not serve without them
...
- [ ] Ad URLs currently point at the client's own site. If you deploy `site/` to **go.summitlinemovers.com**, update the campaigns' Final URLs to match — one destination, everywhere
- [ ] Deploy `site/` to **go.summitlinemovers.com**
- [ ] Ask the client to add one DNS record: `go.summitlinemovers.com` CNAME → your Pages project
...
Generated for Summit Line Movers. 7 campaigns, 1,643.83 USD/day. Campaigns paused, ad groups enabled — so going live is one switch per campaign.
